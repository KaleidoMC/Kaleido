...
public class CarpentryCraftingScreen extends Screen {
    ...
    private static class Entry extends MyList.MyEntry<Entry> implements INestedGuiEventHandler {
        ...
        public void render(int entryIdx, int top, int left, int entryWidth, int entryHeight, int mouseX, int mouseY, boolean hover, float partialTicks) {
            AbstractGui.fill(left, top, left + entryWidth, top + entryHeight, 0x22FFFFFF);
            parent.font.drawString(name, left + 8, top + 4, 0xFFFFFF);
            ...
            parent.drawCenteredString(parent.font, unlocked + "/" + size, (int) ((left + entryWidth - 18) * 1.33), (int) ((top + 5.5) * 1.33), 0xFFFFFF);
            RenderSystem.popMatrix();
            parent.minecraft.getTextureManager().bindTexture(GUI_BARS_TEXTURES);
            AbstractGui.blit(left + entryWidth - 66, top + 5, parent.getBlitOffset(), 0, 0, 32, 5, 32, 32);
            if (unlocked > 0) {
                AbstractGui.blit(left + entryWidth - 66, top + 5, parent.getBlitOffset(), 0, 5, unlocked * 32 / size, 10, 32, 32);
            }
        }
    ...
    private java.util.List<String> tip;
    ...
    protected void init() {
        ...
        tip = font.listFormattedStringToWidth(I18n.format("tip.kaleido.unlock"), 120);
        ...
        addButton(textField = new TextFieldWidget(font, x - 19, y + 1, 38, 18, ""));
        ...
        addButton(confirmBtn = new Button(x + 40, y, 20, 20, "✓", btn -> {
            if (cooldown > 0) {
                return;
            }
            if (selectedButton != null) {
        ...
        addButton(addBtn = new Button(x + 20, y, 20, 20, "+", btn -> {
            addNum(1);
        }));
        addButton(shrinkBtn = new Button(x - 40, y, 20, 20, "-", btn -> {
            addNum(-1);
        }));
        if (selectedButton == null) {
    ...
    public void render(int mouseX, int mouseY, float pTicks) {
        ...
        if (selectedButton != null) {
            ...
            x += 40;
            y += 40 - 62;
            drawCenteredString(font, selectedButton.stack.getDisplayName().getString(), x, y, 0xFFFFFFFF);
            ...
            y += 40;
            for (String s : tip) {
                drawCenteredString(font, s, x, y, 0xFFFFFFFF);
                y += 15;
            }
        }
        ...
        font.drawStringWithShadow(numText, width - 25 - font.getStringWidth(numText), 9, 0xFFFFFFFF);
    }
